...
class MrzParser
{
    ...
    private $non_ISO_3166;
    private $united_nations;
    private $british_nationals;
    ...
    private function stripPadding($str)
    {
        ...
        if (!$str || $str == null) {
            ...
            $return = preg_replace('/\s+/', ' ', $return); // strip double white spaces
            return $return;
        }
    }
    ...
    private function checkDigitVerify($str, $digit)
    {
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if (preg_match('/[A-Za-z<]/', $str[$i], $match)) {
                array_push($numbers, $this->returnCheckDigitValues($str[$i]));
            } else {
                array_push($numbers, (int)$str[$i]);
            }
        }
        ...
        for ($j = 0; $j < count($numbers); $j++) {
            $total += $numbers[$j] * $weighting[$curWeight];
            $curWeight++;
            if ($curWeight == 3) {
                $curWeight = 0;
            }
    ...
    private function parseMRZPassport($mrz)
    {
        ...
        try {
            ...
            $passport['is_valid'] = $checkDigitVerify1 && $checkDigitVerify2 && $checkDigitVerify3 && $checkDigitVerify4 && $checkDigitVerify5;
    ...
    private function parseMrzID1($mrz)
    {
        ...
        try {
            ...
            $id['is_valid'] = $checkDigitVerify1 && $checkDigitVerify2 && $checkDigitVerify3 && $checkDigitVerify4;
    ...
    private function parseMRZVisa($mrz)
    {
        ...
        try {
            ...
            $id['is_valid'] = $checkDigitVerify1 && $checkDigitVerify2 && $checkDigitVerify3;
